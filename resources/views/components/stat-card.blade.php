...
<div {{ $attributes->merge(['class' => 'bg-white rounded-lg shadow-sm border border-gray-200 p-6']) }}>
    <div class="flex items-start justify-between gap-3">
        <div>
            <p class="text-sm font-medium text-slate-500">{{ $displayTitle }}</p>
            <p class="mt-2 font-headline text-2xl font-bold tracking-tight text-slate-900">{{ $value }}</p>
        </div>
        @if($icon)
            <span class="rounded-xl p-2.5 {{ $colorClasses }}">
                @switch($icon)
                    @case('clipboard-list')
